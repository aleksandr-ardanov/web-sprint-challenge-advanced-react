...
Stateful component owns it's own state object and dependent on it's properties, functional component receives props as an input and returns output, so it acts like a functions. Also, stateful component is always a class component.

2. When does a componentWillMount function be called? What about a componentWillUpdate?

componentWillMount called before the render, but we don't have to use it (following by react documentation). same about componentWillUpdate.

We should use componentDidMount and componentDidUpdate respectively

componentDidMount called when component is mounted, componentDidUpdate called when component updates

3. Define stateful logic.

This is the logic which handles changes between state, components and input.
We need it to have our code DRY and reusable.

4. What are the three step of creating a successful test? What is done in each phase?

Arrange - renders a react element into a DOM
Act - query the DOM to get an access to specific element
Assert - makes sure that our element has rendered and visible if needed
